...
public class Car extends Vehicle {
	...
	public void refuel(int amountOfGas) {
		...
		// check if given + remaining amount is greater than the max amount of gas the tank can hold
		// if so, set the remaining to max amount of gas
		// otherwise, add the given amount of gas to the gas remaining
		boolean isGreaterThanMax = amountOfGas + this.gasRemained > this.maxGasAmountInTank;
		if (isGreaterThanMax) {
	...
	public void run(int hour) {
		...
		int amountOfGasWillUse = this.gasConsumedPerHour * hour;

		int gasRemainedAfterRun = this.gasRemained - amountOfGasWillUse;
		if (gasRemainedAfterRun <= 0) {
			System.out.println("Oops, gas out! Please add fuel!");
			this.totalGasConsumed = this.totalGasConsumed + this.gasRemained;
			this.gasRemained = 0;
		} else {
			this.totalGasConsumed = this.totalGasConsumed + amountOfGasWillUse;
			this.gasRemained = this.gasRemained - amountOfGasWillUse;
		}
