...
public class SimpleGame {
	...
	public int digitsSum(int input){
		...
		for(int i = 0; i < charArr.length; i++ ) {
			counter += Integer.parseInt(Character.toString(charArr[i]));
		}
	...
	public static void main(String[] args) {
		...
		System.out.println("Welcome to SimpleGame, which game do you want to play?");
		System.out.println("To convert time press enter 1. To sum digits enter 2.");
		...
		int game = sc.nextInt();
		if (game == 1) {
			System.out.println("Please enter the number of seconds you would like to convert.");
			int seconds = sc.nextInt();
			String result = simplegame.convertTime(seconds);
			System.out.println(result);
		} else {
			System.out.println("Please enter the number you would like to sum.");
			int input = sc.nextInt();
			int sum = simplegame.digitsSum(input);
